...
export default function formater(str) {
    ...
    const format = (/** @type {ASTNode} */pnode) => {
        return pnode.children.map(node => {
            // 不可分割
            switch (node.type) {
                case nodeType.text:
                    return node.value
                case nodeType.table: {
                    ...
                    for (let index = 0; index < maxLen; index++) {
                        const maxChar = Math.max(...allRows.map(item => {
                            return item.children[0]?.raw.text.length || 0
                        }))
