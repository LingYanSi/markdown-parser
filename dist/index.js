...
var helper = {
  ...
  checkIsEnd: function checkIsEnd(tokens, index) {
    var _ref = [tokens[index], tokens[index + 1]],
        currentToken = _ref[0],
        nextToken = _ref[1];
...
function nestList(liList) {
  ...
  liList.forEach(function (item) {
    var deep = Math.floor(item.ident.map(function (i) {
      return i.raw;
    }).join('').length / 4);

    while (stack.length) {
      var relative = stack[stack.length - 1];
      ...
      default:
        {
          ...
          var _ref2 = char === '\\' && nextChar ? [nextChar, 2] : [char, 1];
...
function formatTable(str) {
  var font = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '12px monospace';
  ...
  var ident = str.match(/^\s*/)[0]; // table第二行使用【-】进行补齐，其他使用空格进行补齐

  var spaceChar = ' ',
      splitChar = '-';
  var _ref4 = [getWidth(spaceChar), getWidth(splitChar)],
      spaceW = _ref4[0],
      splitW = _ref4[1];
  var column = [];
  return str.split('\n').map(function (ele) {
    return ele.trim().replace(/^\||\|$/, '').split('|').map(function (i) {
      return i.trim();
    ...
    return ident + row.map(function (ele, index, arr) {
      var i = ele.i,
          width = ele.width;
      var paddingW = column[index] - width;

      var _ref5 = /^-+$/.test(i.trim()) ? [splitChar, Math.round(paddingW / splitW), splitW] : [spaceChar, Math.round(paddingW / spaceW), spaceW],
          _ref6 = _slicedToArray(_ref5, 3),
          padchar = _ref6[0],
          num = _ref6[1],
          perWidth = _ref6[2]; // 如果当前宽度没有被补齐，则同一行下一个字符串的宽度需要被缩短
...
function format(pnode, container) {
  return pnode.children.map(function (node) {
    // 不可分割
    switch (node.type) {
      case nodeType.text:
        return node.value;
      ...
      case nodeType.table:
        {
          var tstr = node.tokens.map(function (i) {
            return i.raw;
          }).join('');
      ...
      case nodeType.ul:
        return fmtList1(node.tokens.map(function (i) {
          return i.raw;
        }).join(''));
      ...
      default:
        return node.tokens.map(function (i) {
          return i.raw;
        }).join('');
    }
  }).join('');
